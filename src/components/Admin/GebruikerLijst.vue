...
export default {
  ...
  methods: {
    ...
    async saveRole(user) {
      try {
        const response = await fetch('https://84.235.165.56:1880/update/role', {
          method: 'PUT',
          headers: {
        ...
        if (response.ok) {
          ...
          addUserAction("Foutmeldingscode " + response.status + " API call {https://84.235.165.56:1880/update/role} in method {saveRole(user)}",
    ...
    async deleteUser(userEmail) {
      try {
        const response = await fetch(`https://84.235.165.56:1880/delete/user`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (response.ok) {
          ...
          addUserAction("Foutmeldingcode " + response.status + " API call {https://84.235.165.56:1880/delete/user} in method {deleteUser(userEmail)}",
